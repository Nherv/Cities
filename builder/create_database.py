...
#CSV resources : [file, id, columns to be read, delimiter type]
CSVs = [
    ['static/cinemas.csv','insee',['cinema_seats'],','],
    ['static/entreprises.csv','Code',['companies2016','newcompanies2016'],';']
    ]
#########################


def load_session():
    """Connect with table cities
    """
...
def build_db(user_arg, user_argtype):
    ...
    Creates rows in the Cities table 
    """
    ...
    for line in reader:
        ...
        if useful_line(get[0], line["libsubcom"], user_arg, user_argtype):
            ...
            new_city = Cities(insee_code= line["codeinsee"],
                postal_code=get[0],
                city=line["libsubcom"],
...
def read(file):
    [csv_file, ide, infos, delimiter] = file
    file = open(csv_file, 'r')
    reader = csv.DictReader(file, delimiter=delimiter)   
    for line in reader:
        for info in infos:
            row = session.query(Cities).filter(Cities.insee_code == line[ide]).update({info: (line[info])})
    session.commit()
    file.close()

def read_all(CSVs):
    """Apply method read to all csv resources
    """
    for file in CSVs:
        read(file)
...
def write_csv():
    ...
    [outcsv.writerow([getattr(curr, column.name)
                      for column in Cities.__table__.columns]) for curr in records]
    outfile.close()
...
def correct():
    rows = session.query(Cities).filter(Cities.party == "NA").all()
    count = session.query(Cities).filter(Cities.party == "NA").count()
